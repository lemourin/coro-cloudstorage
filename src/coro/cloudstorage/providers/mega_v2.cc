...
std::vector<uint8_t> ToIV(std::span<const uint8_t> compkey) {
  auto a32 = MegaV2::ToA32(compkey);
  return MegaV2::ToBytes(std::vector<uint32_t>{a32[4], a32[5], 0, 0});
}

std::vector<uint8_t> ToMAC(std::span<const uint8_t> compkey) {
  auto a32 = MegaV2::ToA32(compkey);
  return MegaV2::ToBytes(std::vector<uint32_t>{a32[6], a32[7]});
}
...
T ToItemImpl(std::span<const uint8_t> master_key, const nlohmann::json& json) {
  ...
  T result = {};
  if constexpr (std::is_same_v<T, MegaV2::File> ||
                std::is_same_v<T, MegaV2::Directory>) {
    ...
    if (item_user == json["u"]) {
      result.compkey = BlockTransform(cipher, MegaV2::FromBase64(item_key));
      if constexpr (std::is_same_v<T, MegaV2::File>) {
        if (result.compkey.size() < 8) {
          throw CloudException("invalid file key");
        }
        result.key = MegaV2::ToFileKey(result.compkey);
      } else {
        result.key = result.compkey;
      }
      try {
        result.attr = MegaV2::DecryptAttribute(
            result.key, MegaV2::FromBase64(std::string(json["a"])));
  ...
  if constexpr (std::is_same_v<T, MegaV2::File>) {
    ...
    if (json.contains("fa")) {
      ...
      if (auto thumbnail = MegaV2::GetAttribute(fa, 0)) {
        result.thumbnail_id = MegaV2::DecodeHandle(*thumbnail);
      }
    }
  }
  ...
  if constexpr (std::is_same_v<T, MegaV2::File> ||
                std::is_same_v<T, MegaV2::Directory>) {
    result.parent = MegaV2::DecodeHandle(std::string(json["p"]));
  }
  result.id = MegaV2::DecodeHandle(std::string(json["h"]));
  return result;
}
...
std::vector<uint8_t> MegaV2::Auth::GetPasswordKey(std::string_view password) {
...
std::string MegaV2::Auth::GetHash(std::string_view text,
...
auto MegaV2::Auth::DecryptSessionId(const std::vector<uint8_t>& passkey,
                                    std::string_view key,
...
auto MegaV2::Auth::GetLoginWithSaltData(std::string_view password,
...
std::optional<std::string_view> MegaV2::GetAttribute(std::string_view attr,
                                                     int index) {
...
std::vector<uint32_t> MegaV2::ToA32(std::span<const uint8_t> bytes) {
...
std::vector<uint8_t> MegaV2::ToBytes(std::span<const uint32_t> span) {
...
std::string_view MegaV2::ToStringView(std::span<const uint8_t> d) {
  return std::string_view(reinterpret_cast<const char*>(d.data()), d.size());
}

std::span<const uint8_t> MegaV2::ToBytes(std::string_view d) {
  return std::span<const uint8_t>(reinterpret_cast<const uint8_t*>(d.data()),
                                  d.size());
}

nlohmann::json MegaV2::DecryptAttribute(std::span<const uint8_t> key,
...
std::string MegaV2::EncryptAttribute(std::span<const uint8_t> key,
                                     const nlohmann::json& json) {
  return EncodeAttributeContent(key, util::StrCat("MEGA", json));
}

std::string MegaV2::ToBase64(std::string_view input) {
...
std::string MegaV2::FromBase64(std::string_view input_sv) {
...
uint64_t MegaV2::DecodeHandle(std::string_view b64) {
...
std::string MegaV2::ToHandle(uint64_t id) {
...
std::string MegaV2::ToAttributeHandle(uint64_t id) {
...
std::string MegaV2::DecodeChunk(std::span<const uint8_t> key,
...
std::string MegaV2::DecodeAttributeContent(std::span<const uint8_t> key,
...
std::string MegaV2::EncodeChunk(std::span<const uint8_t> key,
                                std::span<const uint8_t> compkey,
                                int64_t position, std::string_view input) {
  std::vector<uint8_t> civ = [&] {
    auto iv = ToA32(ToIV(compkey));
...
std::vector<uint8_t> MegaV2::ToFileKey(std::span<const uint8_t> compkey) {
...
std::string MegaV2::EncodeAttributeContent(std::span<const uint8_t> key,
...
std::string MegaV2::BlockEncrypt(std::span<const uint8_t> key,
...
Generator<std::string> MegaV2::GetEncodedStream(
...
auto MegaV2::ToItem(const nlohmann::json& json,
