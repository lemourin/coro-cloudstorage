#ifndef CORO_CLOUDSTORAGE_FUSE_MEGA_V2_H
#define CORO_CLOUDSTORAGE_FUSE_MEGA_V2_H
...
struct MegaV2 {
  ...
  static constexpr std::string_view kId = "mega_v2";
...
class MegaV2::CloudProvider
    : public coro::cloudstorage::CloudProvider<
          MegaV2, CloudProvider<Http, EventLoop, ThumbnailGenerator>> {
 public:
  ...
  const Item* HandleAttributeUpdateEvent(std::string_view attr,
    ...
    if (auto it = items_.find(handle); it != items_.end()) {
      if (auto* file = std::get_if<File>(&it->second)) {
        if (auto thumbnail_attr = MegaV2::GetAttribute(attr, 0)) {
...
class MegaV2::Auth::AuthHandler {
 public:
  AuthHandler(const Http* http, const EventLoop* event_loop,
              MegaV2::Auth::AuthData auth_data)
      : http_(http),
        event_loop_(event_loop),
        auth_data_(std::move(auth_data)) {}

  Task<std::variant<http::Response<>, MegaV2::Auth::AuthToken>> operator()(
    ...
    if (request.method == http::Method::kPost) {
      ...
      if (it1 != std::end(query) && it2 != std::end(query)) {
        ...
        MegaV2::Auth::UserCredential credential = {
            .email = it1->second,
            .password = it2->second,
            .twofactor = it3 != std::end(query)
                             ? std::make_optional(it3->second)
                             : std::nullopt};
        co_return co_await MegaV2::CloudProvider<Http, EventLoop>::GetSession(
 ...
 private:
  const Http* http_;
  const EventLoop* event_loop_;
  MegaV2::Auth::AuthData auth_data_;
};

namespace util {

template <>
inline nlohmann::json ToJson<MegaV2::Auth::AuthToken>(
    MegaV2::Auth::AuthToken token) {
  nlohmann::json json;
...
template <>
inline MegaV2::Auth::AuthToken ToAuthToken<MegaV2::Auth::AuthToken>(
    const nlohmann::json& json) {
  MegaV2::Auth::AuthToken auth_token = {
...
template <>
inline MegaV2::Auth::AuthData GetAuthData<MegaV2>() {
...
#endif  // CORO_CLOUDSTORAGE_FUSE_MEGA_V2_H
